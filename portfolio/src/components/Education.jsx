      ...
      <div className="max-w-5xl mx-auto">
        <div>
          <div className="flex items-center gap-6 mb-16">
            <h2 className="text-4xl md:text-5xl font-bold text-white">Education & Certifications</h2>
                  ...
                  <div key={index} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                    ...
                    <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] bg-slate-900 p-5 rounded-xl border border-slate-800 hover:border-red-500/50 hover:shadow-md hover:shadow-red-900/20 transition-all">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <span className="text-base font-medium text-red-500">{edu.year}</span>
                      </div>
                      <p className="text-slate-300 mb-3 text-lg">{edu.institution}</p>
                      <p className="text-base text-slate-400 font-medium">{edu.score}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div>
              ...
              <div className="space-y-6">
                {certifications.map((cert, index) => (
                  <div key={index} className="bg-slate-900 p-6 rounded-xl border border-slate-800 hover:border-red-500/50 hover:shadow-md hover:shadow-red-900/20 transition-all flex items-start gap-5">
                    <div className="mt-1 p-3 bg-red-500/10 rounded-lg text-red-500 shrink-0">
                      <Award size={28} />
                    </div>
                    <p className="text-slate-300 text-lg leading-relaxed">{cert}</p>
                  </div>
                ))}
              </div>
            </div>
